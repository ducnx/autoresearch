...
def plot_score_with_threshold(model: FaultDetector, data: pd.DataFrame, normal_index: pd.Series = None,
    ...
    Args:
        ...
        show_predicted_anomaly (bool, optional): Whether to show predicted anomalies.
            If True and `normal_index` is provided, both status labels (from `normal_index`) and predicted anomalies
            are shown in the same plot for visual comparison. Defaults to False.
    ...
    if normal_index is not None:
        normal_index = normal_index.reindex(scores.index)
        if normal_index.isna().any():
            raise ValueError('`normal_index` does not cover all score timestamps. Please align the index to `data`.')
        normal_index = normal_index.astype(bool)

    if normal_index is None and not show_predicted_anomaly:
        ax.scatter(scores.index, scores, s=1, alpha=0.8, c=score_color)
    elif normal_index is not None and show_predicted_anomaly:
        predicted_anomalies = predictions.predicted_anomalies.reindex(scores.index).astype(bool)
        ax.scatter(scores.loc[normal_index].index, scores.loc[normal_index], s=1, alpha=0.8, label='normal status',
                   c=score_color)
        ax.scatter(scores.loc[~normal_index].index, scores.loc[~normal_index], s=1, alpha=0.8,
                   label='anomalous status', c=anomaly_color)
        ax.scatter(scores.index[predicted_anomalies], scores[predicted_anomalies], s=12, alpha=0.9, marker='x',
                   linewidths=0.8, label='predicted anomaly', c=anomaly_color)
